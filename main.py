...
max_price = st.sidebar.slider("Max Price",0.0,books_df['Price'].max(), books_df['Price'].max(),0.1)
...
with col1:
    ...
    top_title = filtered_books_df['Name'].value_counts().head(10)
    st.bar_chart(top_title)

with col2:
    st.subheader("Top 10 authors")
    top_author = filtered_books_df['Author'].value_counts().head(10)
...
st.subheader("Number if fiction vs non-fictions books over years")
